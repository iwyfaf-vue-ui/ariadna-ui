...
describe('SidebarMenu.vue', () => {
  ...
  describe('Props', () => {
    ...
    it('cssClass: Should apply custom root class and generate BEM child classes.', async () => {
      ...
      expect(wrapper.find(defaultMock.getSelectorWithDot(_defaultMock.itemsEl)).exists()).toBe(
        true,
      );
      expect(
        wrapper.find(defaultMock.getSelectorWithDot(_defaultMock.itemWrapperEl)).exists(),
      ).toBe(true);
      expect(
        wrapper.find(defaultMock.getSelectorWithDot(_defaultMock.itemDropdownEl)).exists(),
      ).toBe(true);
