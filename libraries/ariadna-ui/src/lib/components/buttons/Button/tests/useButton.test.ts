...
describe('useButton.ts: Basic functionality.', () => {
  it('Should return expected structure.', () => {
    const { emits } = createMockEmits();
    const result = useButton(mockProps, emits);
...
describe('useButton.ts: isDisabled computed.', () => {
  ...
  it('Should be false when neither disabled nor loading is true.', () => {
    const { emits } = createMockEmits();
    const { isDisabled } = useButton(mockProps, emits);

    expect(isDisabled.value).toBe(false);
  });
});

describe('useButton.ts: componentClasses computed.', () => {
...
describe('useButton.ts: clickHandler function.', () => {
  it('Should emit click event when not disabled.', () => {
    const { emits, mocks } = createMockEmits();
    const clickEvent = new MouseEvent('click');
    const { clickHandler } = useButton(mockProps, emits);
