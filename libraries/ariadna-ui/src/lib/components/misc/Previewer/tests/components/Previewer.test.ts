...
describe('Previewer.vue', () => {
  ...
  describe('Props', () => {
    ...
    it('showCodeToggle: Should not render showCode toggle button if showToggle is false.', () => {
      ...
      expect(
        wrapper.find(defaultMock.getSelectorWithDot(defaultMock.actionToggleEl)).exists(),
      ).toBe(false);
    });
    ...
    it('cssClass: Should apply custom root class and generate BEM child classes.', async () => {
      ...
      expect(
        wrapper.find(defaultMock.getSelectorWithDot(_defaultMock.descriptionEl)).exists(),
      ).toBe(true);
      expect(wrapper.find(defaultMock.getSelectorWithDot(_defaultMock.componentEl)).exists()).toBe(
        true,
      );
      expect(wrapper.find(defaultMock.getSelectorWithDot(_defaultMock.actionEl)).exists()).toBe(
        true,
      );
      expect(
        wrapper.find(defaultMock.getSelectorWithDot(_defaultMock.actionToggleEl)).exists(),
      ).toBe(true);
