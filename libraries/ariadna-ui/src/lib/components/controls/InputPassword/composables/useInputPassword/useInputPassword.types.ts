...
/**
 * @description
 * Return type for the `useInputPassword` composable function.
 * Contains reactive properties and methods for InputPassword component functionality.
 */
export type TUseInputPasswordReturn = {
  /**
   * State of password visibility toggle button.
   */
  ...
  /**
   * Reactive type of the input element that reflects whether the password is currently shown (`text`) or masked
   * (`password`). The value updates automatically when `togglePassword()` is called.
   */
  ...
  /**
   * Toggles the current visibility state of the password field. When invoked, it flips inputType between `password`
   * and `text`.
   */
  togglePassword: () => void;
