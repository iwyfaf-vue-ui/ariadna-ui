...
import type { TUseInputPasswordReturn } from './useInputPassword.types';
import type {
  TInputPasswordProps,
  TInputPasswordSlots,
  TInputPasswordEmits,
...
export default function useInputPassword(
  ...
  slots: TInputPasswordSlots,
  emits: TInputPasswordEmits,
  openMeterPanel: () => void,
  closeMeterPanel: () => void,
): TUseInputPasswordReturn {
  const id = useId();
  const focused = ref(false);
  const hovered = ref(false);
  const showPassword = ref<boolean>(props.showPassword!);
