...
describe('InputText.vue: Basic render.', () => {
...
describe('InputText.vue: Props.', () => {
  ...
  it('id: Should set input id from id prop if provided.', () => {
    ...
    expect(wrapper.find(defaultMock.getSelectorWithDot(defaultMock.inputEl)).attributes('id')).toBe(
      defaultMock.idProp,
    );
  });
  ...
  it('autocomplete: Should set input autocomplete from prop.', () => {
    ...
    expect(
      wrapper.find(defaultMock.getSelectorWithDot(defaultMock.inputEl)).attributes('autocomplete'),
    ).toBe('on');
  ...
  it('Should add valid class on root if valid prop is true.', () => {
    const wrapper = mount(InputText, {
      props: {
        modelValue: null,
        valid: true,
  ...
  it('Should add invalid class on root if invalid prop is true.', () => {
    const wrapper = mount(InputText, {
      props: {
        modelValue: null,
        invalid: true,
  ...
  it('errors: Should render errors if prop invalid and prop errors provided.', () => {
    ...
    expect(wrapper.findAll(`${defaultMock.errorsEl}-1, ${defaultMock.errorsEl}-2`).length).toBe(2);
  ...
  it('cssClass: Should apply custom root class and generate BEM child classes.', async () => {
    ...
    expect(wrapper.find(defaultMock.getSelectorWithDot(_defaultMock.labelEl)).exists()).toBe(true);
    expect(wrapper.find(defaultMock.getSelectorWithDot(_defaultMock.inputEl)).exists()).toBe(true);
    expect(wrapper.find(defaultMock.getSelectorWithDot(_defaultMock.placeholderEl)).exists()).toBe(
      true,
    );
    expect(wrapper.find(defaultMock.getSelectorWithDot(_defaultMock.errorsEl)).exists()).toBe(true);
    ...
    expect(wrapper.find(defaultMock.getSelectorWithDot(_defaultMock.validModifier)).exists()).toBe(
      true,
    );
    expect(
...
describe('InputText.vue: Accessibility.', () => {
