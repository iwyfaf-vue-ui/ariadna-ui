...
import { ChipsSelectorTestData } from '../../tests/test-data/Chips.selector.test-data';

const defaultMock = new ChipsSelectorTestData();

function mountWithComposable(props: TChipsProps, route = { path: '/' }) {
  const emits = vi.fn();

  return mount(
    defineComponent({
      setup() {
        const result = useChips(props, emits);
        return { ...result, emits };
      },
      render() {
        return h('div');
...
describe('useInputText', () => {
  describe('Basic functionality', () => {
    it('Should return expected structure.', () => {
      ...
      expect(vm).toHaveProperty('focusedIdx');
