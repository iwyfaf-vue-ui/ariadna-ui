...
import SelectMultipleGroup from '../../SelectMultipleGroup.vue';
...
describe('SelectMultipleGroup', () => {
  describe('Basic render', () => {
    it('Should render root element with correct class.', () => {
      const wrapper = mount(SelectMultipleGroup, {
  ...
  describe('Props', () => {
    it('modelValue: Should update input value when prop.modelValue changes.', async () => {
      const wrapper = mount(SelectMultipleGroup, {
        props: {
          ...defaultMock.mockProps,
          modelValue: [defaultMock.mockProps.options[0]],
        },
      });

      await wrapper.setProps({ modelValue: [defaultMock.mockProps.options[1]] });
      expect(wrapper.props('modelValue')).toStrictEqual([defaultMock.mockProps.options[1]]);
    });
    ...
    it('optionLabel: Should display custom label property for options.', async () => {
      ...
      await nextTick();
      expect(wrapper.find(defaultMock.textSelectedEl).text()).toBe(
    ...
    it('placeholder: Should display placeholder if modelValue is null.', () => {
      ...
      expect(wrapper.find(defaultMock.textSelectedEl).text()).toBe(defaultMock.placeholderProp);
    });
    ...
    it('cssClass: Should apply custom root class and generate BEM child classes.', async () => {
      const _defaultMock = new SelectMultipleGroupSelectorTestData(defaultMock.cssClassProp);
      const errors = ['Error 1', 'Error 2'];
      ...
      expect(
        wrapper.find(defaultMock.getSelectorWithDot(_defaultMock.textSelectedEl)).exists(),
      ).toBe(true);
  ...
  describe('Slots', () => {
    it('label: Should render default label slot.', async () => {
      ...
      expect(wrapper.find(defaultMock.textSelectedEl).element.innerHTML).toBe(
        (await SelectMultipleGroupSelectorTestData.getSelectMultipleGroupLabelSlotDefault()).trim(),
      );
    });
    ...
    it('label: Should render custom label slot if provided.', async () => {
      const wrapper = mount(SelectMultipleGroup, {
        ...
        slots: {
          label: (
            await SelectMultipleGroupSelectorTestData.getSelectMultipleGroupLabelSlotCustom()
          ).trim(),
        },
      });
      ...
      expect(wrapper.find(defaultMock.textSelectedEl).exists()).toBeTruthy();
      expect(wrapper.find(defaultMock.textSelectedEl).element.innerHTML).toBe(
        (await SelectMultipleGroupSelectorTestData.getSelectMultipleGroupLabelSlotCustom()).trim(),
      );
    });
    ...
    it('optionsGroup: Should render default optionsGroup slot.', async () => {
      ...
      expect(wrapper.find(defaultMock.optionsGroupEl).element.innerHTML).toBe(
        (
          await SelectMultipleGroupSelectorTestData.getSelectMultipleGroupOptionsGroupSlotDefault()
        ).trim(),
      );
    });
    ...
    it('optionsGroup: Should render custom optionsGroup slot if provided.', async () => {
      ...
      expect(wrapper.find(defaultMock.optionsGroupEl).element.innerHTML).toBe(
        (
          await SelectMultipleGroupSelectorTestData.getSelectMultipleGroupOptionsGroupSlotCustom()
        ).trim(),
      );
    });
    ...
    it('options: Should render default options slot.', async () => {
      ...
      expect(wrapper.find(defaultMock.optionsEl).element.innerHTML).toBe(
        (
          await SelectMultipleGroupSelectorTestData.getSelectMultipleGroupOptionsSlotDefault()
        ).trim(),
      );
    });
    ...
    it('options: Should render custom options slot if provided.', async () => {
      ...
      expect(wrapper.find(defaultMock.optionsEl).element.innerHTML).toBe(
        (
          await SelectMultipleGroupSelectorTestData.getSelectMultipleGroupOptionsSlotCustom()
        ).trim(),
      );
    });
    ...
    it('empty: Should render default empty slot.', async () => {
      ...
      expect(wrapper.find(defaultMock.emptyOptionsEl).element.innerHTML).toBe(
        (await SelectMultipleGroupSelectorTestData.getSelectMultipleGroupEmptySlotDefault()).trim(),
      );
    });
    ...
    it('empty: Should render custom empty slot if provided.', async () => {
      const wrapper = mount(SelectMultipleGroup, {
        ...
        slots: {
          empty: await SelectMultipleGroupSelectorTestData.getSelectMultipleGroupEmptySlotCustom(),
        },
      });

      expect(wrapper.find(defaultMock.emptyOptionsEl).exists()).toBeTruthy();
      expect(wrapper.find(defaultMock.emptyOptionsEl).element.innerHTML).toBe(
        (await SelectMultipleGroupSelectorTestData.getSelectMultipleGroupEmptySlotCustom()).trim(),
      );
    });
    ...
    it('tiles: Should render default tiles slot.', async () => {
      ...
      expect(wrapper.find(defaultMock.tileEl).element.innerHTML).toBe(
        (await SelectMultipleGroupSelectorTestData.getSelectMultipleGroupTilesSlotDefault()).trim(),
      );
    });
    ...
    it('tiles: Should render custom tiles slot if provided.', async () => {
      const wrapper = mount(SelectMultipleGroup, {
        ...
        slots: {
          tiles: await SelectMultipleGroupSelectorTestData.getSelectMultipleGroupTilesSlotCustom(),
        },
      });

      expect(wrapper.find(defaultMock.textSelectedEl).exists()).toBeTruthy();
      expect(wrapper.find(defaultMock.textSelectedEl).element.innerHTML).toBe(
        (await SelectMultipleGroupSelectorTestData.getSelectMultipleGroupTilesSlotCustom()).trim(),
      );
    });
    ...
    it('multiselectCheckbox: Should render default multiselectCheckbox slot.', async () => {
      ...
      expect(wrapper.find(defaultMock.filterCheckboxEl).element.innerHTML).toBe(
        (
          await SelectMultipleGroupSelectorTestData.getSelectMultipleGroupMultiselectCheckboxSlotDefault()
        ).trim(),
      );
    });
    ...
    it('multiselectCheckbox: Should render custom multiselectCheckbox slot if provided.', async () => {
      const wrapper = mount(SelectMultipleGroup, {
        ...
        slots: {
          multiselectCheckbox:
            await SelectMultipleGroupSelectorTestData.getSelectMultipleGroupMultiselectCheckboxSlotCustom(),
        },
      });
    ...
    it('filterInput: Should render filterInput empty slot.', async () => {
      ...
      expect(wrapper.find(defaultMock.filterEl).element.innerHTML).toBe(
        (
          await SelectMultipleGroupSelectorTestData.getSelectMultipleGroupFilterInputSlotDefault()
        ).trim(),
      );
    });
    ...
    it('filterInput: Should render custom filterInput slot if provided.', async () => {
      ...
      expect(wrapper.find(defaultMock.filterEl).element.innerHTML).toBe(
        (
          await SelectMultipleGroupSelectorTestData.getSelectMultipleGroupFilterInputSlotCustom()
        ).trim(),
      );
    });
    ...
    it('emptyFilter: Should render emptyFilter empty slot.', async () => {
      ...
      expect(wrapper.find(defaultMock.emptyFilterEl).element.innerHTML).toBe(
        (
          await SelectMultipleGroupSelectorTestData.getSelectMultipleGroupEmptyFilterSlotDefault()
        ).trim(),
      );
    });
    ...
    it('emptyFilter: Should render custom emptyFilter slot if provided.', async () => {
      const wrapper = mount(SelectMultipleGroup, {
        ...
        slots: {
          emptyFilter: (
            await SelectMultipleGroupSelectorTestData.getSelectMultipleGroupEmptyFilterSlotCustom()
          ).trim(),
        },
      });
    ...
    it('filterIcon: Should render filterIcon empty slot.', async () => {
      ...
      expect(wrapper.find(defaultMock.filterEl).element.innerHTML).toBe(
        (
          await SelectMultipleGroupSelectorTestData.getSelectMultipleGroupFilterIconSlotDefault()
        ).trim(),
      );
    });
    ...
    it('filterIcon: Should render custom filterIcon empty slot.', async () => {
      ...
      expect(wrapper.find(defaultMock.filterEl).element.innerHTML).toBe(
        (
          await SelectMultipleGroupSelectorTestData.getSelectMultipleGroupFilterIconSlotCustom()
        ).trim(),
      );
    });
    ...
    it('toggleIcon: Should render toggleIcon empty slot.', async () => {
      ...
      expect(wrapper.find(defaultMock.toggleIconEl).element.innerHTML).toBe(
        (
          await SelectMultipleGroupSelectorTestData.getSelectMultipleGroupToggleIconSlotDefault()
        ).trim(),
      );

      await wrapper.setProps({ modelValue: [defaultMock.optionsExample()[0].children[0]] });
    ...
    it('toggleIcon: Should render custom toggleIcon empty slot.', async () => {
      const wrapper = mount(SelectMultipleGroup, {
        ...
        slots: {
          toggleIcon: (
            await SelectMultipleGroupSelectorTestData.getSelectMultipleGroupToggleIconSlotCustom()
          ).trim(),
        },
      });
    ...
    it('cleanIcon: Should render cleanIcon empty slot.', async () => {
      const wrapper = mount(SelectMultipleGroup, {
        ...
        slots: {
          cleanIcon: (
            await SelectMultipleGroupSelectorTestData.getSelectMultipleGroupCleanIconSlotCustom()
          ).trim(),
        },
      });
    ...
    it('loadingIcon: Should render loadingIcon empty slot.', async () => {
      ...
      expect(wrapper.find(defaultMock.loadingEl).element.innerHTML).toBe(
        (
          await SelectMultipleGroupSelectorTestData.getSelectMultipleGroupLoadingIconSlotDefault()
        ).trim(),
      );
    });
    ...
    it('loadingIcon: Should render custom loadingIcon empty slot.', async () => {
      const wrapper = mount(SelectMultipleGroup, {
        ...
        slots: {
          loadingIcon: (
            await SelectMultipleGroupSelectorTestData.getSelectMultipleGroupLoadingIconSlotCustom()
          ).trim(),
        },
      });
    ...
    it('loading: Should render loading empty slot.', async () => {
      ...
      expect(wrapper.find(defaultMock.listLoadingEl).element.innerHTML).toBe(
        (
          await SelectMultipleGroupSelectorTestData.getSelectMultipleGroupLoadingSlotDefault()
        ).trim(),
      );
    });
    ...
    it('loading: Should render custom loading empty slot.', async () => {
      const wrapper = mount(SelectMultipleGroup, {
        ...
        slots: {
          loading: (
            await SelectMultipleGroupSelectorTestData.getSelectMultipleGroupLoadingSlotCustom()
          ).trim(),
        },
      });
    ...
    it('errors: Should render errors slot if provided.', async () => {
      ...
      expect(wrapper.find(defaultMock.errorsEl).element.innerHTML).toBe(
        (
          await SelectMultipleGroupSelectorTestData.getSelectMultipleGroupErrorsSlotDefault()
        ).trim(),
      );
    });
    ...
    it('errors: Should render custom errors empty slot.', async () => {
      ...
      const wrapper = mount(SelectMultipleGroup, {
        ...
        slots: {
          errors:
            await SelectMultipleGroupSelectorTestData.getSelectMultipleGroupErrorsSlotCustom(),
        },
      });

      expect(wrapper.find(defaultMock.errorsEl).exists()).toBeTruthy();
      expect(wrapper.find(defaultMock.errorsEl).element.innerHTML).toBe(
        (await SelectMultipleGroupSelectorTestData.getSelectMultipleGroupErrorsSlotCustom()).trim(),
      );
    });
  });
  ...
  describe('Emits', () => {
    it('update:model-value: Should emit update:modelValue on input.', async () => {
      const wrapper = mount(SelectMultipleGroup, {
        props: {
          ...defaultMock.mockProps,
          'onUpdate:modelValue': (e: TSelectMultipleGroupProps['modelValue']) =>
    ...
    it('update:filter-value: Should emit update:model-value-filter on filter input.', async () => {
      const wrapper = mount(SelectMultipleGroup, {
        props: {
          ...defaultMock.mockProps,
          filter: defaultMock.filterProp,
          'onUpdate:filterValue': (e: TSelectMultipleGroupProps['filterValue']) =>
  ...
  describe('Accessibility: Keyboard Support', () => {
    ...
    it('Checking the selection and removal of an option by key Space.', async () => {
      ...
      expect(wrapper.find(defaultMock.textSelectedEl).element.textContent).toBe(
