...
type MountParams = {
  propsOverride?: Partial<TSelectMultipleProps>;
  vModelValue?: any[];
  options?: any[];
  selectedOption?: any;
...
function mountWithComposable(params: MountParams = {}) {
  ...
  const props = { ...defaultMock.mockProps, ...propsOverride } as TSelectMultipleProps;
...
describe('useSelectMultipleScroll', () => {
  let options: TSelectMultipleProps['options'];
  ...
  describe('Virtual scroller scrolling', () => {
    it('Should call virtualScrollerRef.scrollTo with selectedIndex when virtualScroller prop is set.', async () => {
      ...
      const { opened } = mountWithComposable({
        propsOverride: {
          virtualScroller: { itemHeight: 40 } as TSelectMultipleProps['virtualScroller'],
        },
        vModelValue: [selectedOption],
        options,
        selectedOption,
