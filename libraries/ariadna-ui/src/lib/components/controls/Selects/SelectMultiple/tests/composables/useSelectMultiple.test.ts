...
import { SelectMultipleSelectorTestData } from '../../tests/test-data/SelectMultiple.selector.test-data';

const defaultMock = new SelectMultipleSelectorTestData();

function mountWithComposable(props: TSelectMultipleProps, route = { path: '/' }) {
  const emits = vi.fn();

  return mount(
    defineComponent({
      setup() {
        const result = useSelectMultiple(props, emits);
        return { ...result, emits };
      },
      render() {
        return h('div');
