...
type MountParams = {
  propsOverride?: Partial<TSelectSingleFlatProps>;
  vModelValue?: any;
  options?: any[];
  selectedOption?: any;
...
function mountWithComposable(params: MountParams = {}) {
  ...
  const props = { ...defaultMock.mockProps, ...propsOverride } as TSelectSingleFlatProps;
...
describe('useSelectSingleFlatScroll', () => {
  let options: TSelectSingleFlatProps['options'];
  ...
  describe('Virtual scroller scrolling', () => {
    it('Should call virtualScrollerRef.scrollTo with selectedIndex when virtualScroller prop is set.', async () => {
      ...
      const { opened } = mountWithComposable({
        propsOverride: {
          virtualScroller: { itemHeight: 40 } as TSelectSingleFlatProps['virtualScroller'],
        },
        vModelValue: selectedOption,
        options,
        selectedOption,
