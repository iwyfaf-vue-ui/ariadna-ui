...
import SelectSingleFlat from '../../SelectSingleFlat.vue';
import { SelectSingleFlatSelectorTestData } from '../test-data/SelectSingleFlat.selector.test-data';
import type { TSelectSingleFlatProps } from '../../SelectSingleFlat';

const defaultMock = new SelectSingleFlatSelectorTestData();

describe('SelectSingleFlat', () => {
  describe('Basic render', () => {
    it('Should render root element with correct class.', () => {
      const wrapper = mount(SelectSingleFlat, {
  ...
  describe('Props', () => {
    ...
    it('cssClass: Should apply custom root class and generate BEM child classes.', async () => {
      const _defaultMock = new SelectSingleFlatSelectorTestData(defaultMock.cssClassProp);
      const errors = ['Error 1', 'Error 2'];
  ...
  describe('Slots', () => {
    it('options: Should render default options slot.', async () => {
      ...
      expect(wrapper.find(defaultMock.optionsEl).element.innerHTML).toBe(
        (await SelectSingleFlatSelectorTestData.getSelectSingleFlatOptionsSlotDefault()).trim(),
      );
    });
    ...
    it('options: Should render custom options slot if provided.', async () => {
      ...
      expect(wrapper.find(defaultMock.optionsEl).exists()).toBeTruthy();
      expect(wrapper.find(defaultMock.optionsEl).element.innerHTML).toBe(
        (await SelectSingleFlatSelectorTestData.getSelectSingleFlatOptionsSlotCustom()).trim(),
      );
    });
    ...
    it('empty: Should render default empty slot.', async () => {
      ...
      expect(wrapper.find(defaultMock.emptyOptionsEl).element.innerHTML).toBe(
        (await SelectSingleFlatSelectorTestData.getSelectSingleFlatEmptySlotDefault()).trim(),
      );
    });
    ...
    it('empty: Should render custom empty slot if provided.', async () => {
      const wrapper = mount(SelectSingleFlat, {
        ...
        slots: {
          empty: await SelectSingleFlatSelectorTestData.getSelectSingleFlatEmptySlotCustom(),
        },
      });

      expect(wrapper.find(defaultMock.emptyOptionsEl).exists()).toBeTruthy();
      expect(wrapper.find(defaultMock.emptyOptionsEl).element.innerHTML).toBe(
        (await SelectSingleFlatSelectorTestData.getSelectSingleFlatEmptySlotCustom()).trim(),
      );
    });
    ...
    it('filterInput: Should render filterInput empty slot.', async () => {
      ...
      expect(wrapper.find(defaultMock.filterEl).element.innerHTML).toBe(
        (await SelectSingleFlatSelectorTestData.getSelectSingleFlatFilterInputSlotDefault()).trim(),
      );
    });
    ...
    it('filterInput: Should render custom filterInput slot if provided.', async () => {
      ...
      expect(wrapper.find(defaultMock.filterEl).exists()).toBeTruthy();
      expect(wrapper.find(defaultMock.filterEl).element.innerHTML).toBe(
        (await SelectSingleFlatSelectorTestData.getSelectSingleFlatFilterInputSlotCustom()).trim(),
      );
    });
    ...
    it('emptyFilter: Should render emptyFilter empty slot.', async () => {
      ...
      expect(wrapper.find(defaultMock.emptyFilterEl).element.innerHTML).toBe(
        (await SelectSingleFlatSelectorTestData.getSelectSingleFlatEmptyFilterSlotDefault()).trim(),
      );
    });
    ...
    it('emptyFilter: Should render custom emptyFilter slot if provided.', async () => {
      const wrapper = mount(SelectSingleFlat, {
        ...
        slots: {
          emptyFilter: (
            await SelectSingleFlatSelectorTestData.getSelectSingleFlatEmptyFilterSlotCustom()
          ).trim(),
        },
      });
      ...
      expect(wrapper.find(defaultMock.emptyFilterEl).exists()).toBeTruthy();
      expect(wrapper.find(defaultMock.emptyFilterEl).element.innerHTML).toBe(
        (await SelectSingleFlatSelectorTestData.getSelectSingleFlatEmptyFilterSlotCustom()).trim(),
      );
    });
    ...
    it('filterIcon: Should render filterIcon empty slot.', async () => {
      ...
      expect(wrapper.find(defaultMock.filterEl).exists()).toBeTruthy();
      expect(wrapper.find(defaultMock.filterEl).element.innerHTML).toBe(
        (await SelectSingleFlatSelectorTestData.getSelectSingleFlatFilterIconSlotDefault()).trim(),
      );
    });
    ...
    it('filterIcon: Should render custom filterIcon empty slot.', async () => {
      ...
      expect(wrapper.find(defaultMock.filterEl).exists()).toBeTruthy();
      expect(wrapper.find(defaultMock.filterEl).element.innerHTML).toBe(
        (await SelectSingleFlatSelectorTestData.getSelectSingleFlatFilterIconSlotCustom()).trim(),
      );
    });
    ...
    it('toggleIcon: Should render toggleIcon empty slot.', async () => {
      ...
      expect(wrapper.find(defaultMock.toggleIconEl).element.innerHTML).toBe(
        (await SelectSingleFlatSelectorTestData.getSelectSingleFlatToggleIconSlotDefault()).trim(),
      );
    ...
    it('toggleIcon: Should render custom toggleIcon empty slot.', async () => {
      const wrapper = mount(SelectSingleFlat, {
        ...
        slots: {
          toggleIcon: (
            await SelectSingleFlatSelectorTestData.getSelectSingleFlatToggleIconSlotCustom()
          ).trim(),
        },
      });

      expect(wrapper.find(defaultMock.toggleIconEl).element.innerHTML).toBe(
        (await SelectSingleFlatSelectorTestData.getSelectSingleFlatToggleIconSlotCustom()).trim(),
      );
    ...
    it('cleanIcon: Should render cleanIcon empty slot.', async () => {
      const wrapper = mount(SelectSingleFlat, {
        ...
        slots: {
          cleanIcon: (
            await SelectSingleFlatSelectorTestData.getSelectSingleFlatCleanIconSlotCustom()
          ).trim(),
        },
      });
      ...
      expect(wrapper.find(defaultMock.cleanIconEl).element.innerHTML).toBe(
        (await SelectSingleFlatSelectorTestData.getSelectSingleFlatCleanIconSlotCustom()).trim(),
      );
    });
    ...
    it('loadingIcon: Should render loadingIcon empty slot.', async () => {
      ...
      expect(wrapper.find(defaultMock.loadingEl).element.innerHTML).toBe(
        (await SelectSingleFlatSelectorTestData.getSelectSingleFlatLoadingIconSlotDefault()).trim(),
      );
    });
    ...
    it('loadingIcon: Should render custom loadingIcon empty slot.', async () => {
      const wrapper = mount(SelectSingleFlat, {
        ...
        slots: {
          loadingIcon: (
            await SelectSingleFlatSelectorTestData.getSelectSingleFlatLoadingIconSlotCustom()
          ).trim(),
        },
      });
      ...
      expect(wrapper.find(defaultMock.loadingEl).exists()).toBeTruthy();
      expect(wrapper.find(defaultMock.loadingEl).element.innerHTML).toBe(
        (await SelectSingleFlatSelectorTestData.getSelectSingleFlatLoadingIconSlotCustom()).trim(),
      );
    });
    ...
    it('loading: Should render loading empty slot.', async () => {
      ...
      expect(wrapper.find(defaultMock.listLoadingEl).element.innerHTML).toBe(
        (await SelectSingleFlatSelectorTestData.getSelectSingleFlatLoadingSlotDefault()).trim(),
      );
    });
    ...
    it('loading: Should render custom loading empty slot.', async () => {
      const wrapper = mount(SelectSingleFlat, {
        ...
        slots: {
          loading: (
            await SelectSingleFlatSelectorTestData.getSelectSingleFlatLoadingSlotCustom()
          ).trim(),
        },
      });
      ...
      expect(wrapper.find(defaultMock.listLoadingEl).exists()).toBeTruthy();
      expect(wrapper.find(defaultMock.listLoadingEl).element.innerHTML).toBe(
        (await SelectSingleFlatSelectorTestData.getSelectSingleFlatLoadingSlotCustom()).trim(),
      );
    });
    ...
    it('errors: Should render errors slot if provided.', async () => {
      ...
      expect(wrapper.find(defaultMock.errorsEl).element.innerHTML).toBe(
        (await SelectSingleFlatSelectorTestData.getSelectSingleFlatErrorsSlotDefault()).trim(),
      );
    });
    ...
    it('errors: Should render custom errors empty slot.', async () => {
      ...
      const wrapper = mount(SelectSingleFlat, {
        ...
        slots: {
          errors: await SelectSingleFlatSelectorTestData.getSelectSingleFlatErrorsSlotCustom(),
        },
      });

      expect(wrapper.find(defaultMock.errorsEl).exists()).toBeTruthy();
      expect(wrapper.find(defaultMock.errorsEl).element.innerHTML).toBe(
        (await SelectSingleFlatSelectorTestData.getSelectSingleFlatErrorsSlotCustom()).trim(),
      );
    });
  });
  ...
  describe('Emits', () => {
    it('update:model-value: Should emit update:modelValue on input.', async () => {
      const wrapper = mount(SelectSingleFlat, {
        props: {
          ...defaultMock.mockProps,
          'onUpdate:modelValue': (e: TSelectSingleFlatProps['modelValue']) =>
    ...
    it('update:filter-value: Should emit update:model-value-filter on filter input.', async () => {
      const wrapper = mount(SelectSingleFlat, {
        props: {
          ...defaultMock.mockProps,
          filter: true,
          'onUpdate:filterValue': (e: TSelectSingleFlatProps['filterValue']) =>
