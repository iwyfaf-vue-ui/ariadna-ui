...
import { SelectSingleGroupSelectorTestData } from '../../tests/test-data/SelectSingleGroup.selector.test-data';

const defaultMock = new SelectSingleGroupSelectorTestData();

function mountWithComposable(props: TSelectSingleGroupProps, route = { path: '/' }) {
  const emits = vi.fn();

  return mount(
    defineComponent({
      setup() {
        const result = useSelectSingleGroup(props, emits);
        return { ...result, emits };
      },
      render() {
        return h('div');
