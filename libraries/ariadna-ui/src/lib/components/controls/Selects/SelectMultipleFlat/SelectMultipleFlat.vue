<template>
  <div ref="selectMultipleFlat" :class="componentClasses">
                ...
                >
                  <div :class="`${props.cssClass}__option-checkbox`">
                    <slot name="options" :option="item.data" :checked="isSelected(item.data)">
                      <Checkbox :model-value="isSelected(item.data)" :aria-label="item.data" />
...
const props = withDefaults(defineProps<TSelectMultipleFlatProps>(), {
  modelValue: () => [],
...
const slots = defineSlots<TSelectMultipleFlatSlots>();
const emits = defineEmits<TSelectMultipleFlatEmits>();
const vModel = defineModel<NonNullable<TSelectMultipleFlatProps['modelValue']>>('modelValue', {
  default: [],
});
const filterModel = defineModel<NonNullable<TSelectMultipleFlatProps['filterValue']>>(
  'filterValue',
  {
    default: '',
  },
...
const selectMultipleFlatRef = useTemplateRef('selectMultipleFlat');
...
const { calculate, top, left } = usePosition(selectMultipleFlatRef, optionsListRef, {
  ...usePositionDefaultOptions,
...
const { filterOptions, onFilter } = useSelectsFlatFilteredOptions<
  TSelectMultipleFlatProps['options']
>(
...
const {
  ...
  onExpandAfterEnter,
  onExpandBeforeLeave,
} = useSelectMultipleFlat(props, emits);

const { multiselectCheckboxChecked, onChangeMultiselectCheckbox } = useSelectMultipleFlatCheckbox(
  vModel,
  props,
);
...
const {
  ...
  onClickItem,
  isSelected,
} = useSelectMultipleFlatActions(props, vModel, calculate, opened);
