...
import { SelectMultipleFlatSelectorTestData } from '../../tests/test-data/SelectMultipleFlat.selector.test-data';

const defaultMock = new SelectMultipleFlatSelectorTestData();

function mountWithComposable(props: TSelectMultipleFlatProps, route = { path: '/' }) {
  const emits = vi.fn();

  return mount(
    defineComponent({
      setup() {
        const result = useSelectMultipleFlat(props, emits);
        return { ...result, emits };
      },
      render() {
        return h('div');
