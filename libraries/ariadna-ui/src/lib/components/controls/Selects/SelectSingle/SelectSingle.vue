<template>
  <div ref="selectSingle" :class="componentClasses">
...
import { ESelectSingleNumberConfig, ESelectSinglePropsDefault } from './types/SelectSingle.enums';
...
const props = withDefaults(defineProps<TSelectSingleProps>(), {
  modelValue: null,
...
const slots = defineSlots<TSelectSingleSlots>();
const emits = defineEmits<TSelectSingleEmits>();
const vModel = defineModel<TSelectSingleProps['modelValue']>();
const filterModel = defineModel<NonNullable<TSelectSingleProps['filterValue']>>('filterValue', {
  default: '',
});
...
const {
  ...
  cleanSelectedData,
  onClickItem,
} = useSelectSingleActions(props, vModel, calculate, opened);
