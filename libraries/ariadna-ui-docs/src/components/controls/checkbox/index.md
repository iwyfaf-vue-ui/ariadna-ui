...
| `errors`    | Используется для отображения ошибок `input` поля.          |

### `default`

- **Описание:** Используется для отображения контента.
- **Тип:** `() => VNode[]`
...
- **Описание:** Используется для отображения дополнительного содержимого при реализации кастомного 
`<input type="checkbox">`, совместно с props [custom](#custom)
- **Тип:** `() => VNode[]`
