  ...
  <div>
    ...
    >
      ...
      <template #galleryImageItem="{ imageItem }">
        <img :src="imageItem.source" width="40px" height="40px" alt="Image" />
      </template>
      ...
      <template #sliderItem="{ mediaItem, registerIframe }">
        <img v-if="mediaItem.type === 'IMAGE'" :src="mediaItem.source" :alt="mediaItem.alt" />
      </template>
...
function useViewerComponent() {
  ...
  function open() {
    ...
    viewerApi.open();
    viewerApi.setSrcKey('source');
    viewerApi.setGallery(imageGallery.map((item) => ({ source: item.src })));
  }
