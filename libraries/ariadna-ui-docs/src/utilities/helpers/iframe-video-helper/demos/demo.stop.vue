  ...
  <div>
    <div v-for="(item, idx) in iframesData" :key="item.label" style="margin-bottom: 24px">
      ...
      <div style="display: flex; flex-direction: column; margin-top: 8px">
        ...
        <Button v-if="registered[idx]" @click="playIframe(iframeRefs[idx])" modifier="success">
          Play
        </Button>

        <Button v-if="registered[idx]" @click="stopIframe(iframeRefs[idx])" modifier="danger">
          Stop
        </Button>
      </div>
    </div>
...
function playIframe(el: HTMLIFrameElement | null) {
  if (!el) {
    return;
  }
...
function stopIframe(el: HTMLIFrameElement | null) {
  if (!el) {
    return;
  }
